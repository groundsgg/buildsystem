...
dependencies {
    ...
    // Paper, not Spigot. Upstream ships spigot-only on purpose; this fork runs on Paper and
    // wants its API (async chunk loading and the scheduler the registry pull leans on).
...
tasks.runServer {
    ...
    minecraftVersion(libs.versions.paperapi.get().substringBefore(".build"))
}
